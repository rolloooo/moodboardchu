...
export default function MoodCard({ quote, imageUrl, mood }: MoodCardProps) {
  ...
  // Default to green if mood not found
  const color = mood ? moodColors[mood] || "border-green-400 text-green-400" : "border-green-400 text-green-400";

  return (
    <div className={`border-2 ${color} bg-black/90 backdrop-blur p-6 space-y-4`}>
      {/* Mood header */}
